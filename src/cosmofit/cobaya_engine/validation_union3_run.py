"""Validation run for flat LCDM with Union3."""
...
def main(argv: list[str] | None = None) -> int:
    """Run flat LCDM against the official Union3 likelihood."""

    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("outputs/validation_union3_lcdm"),
        help="Parent directory for Union3 validation artifacts.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    run_config = RunConfig(
        ...
        runtime=RuntimeConfig(
            run_label="validation-union3-lcdm",
...
def _default_run_label() -> str:
    ...
    return "validation-union3-lcdm-" + timestamp
