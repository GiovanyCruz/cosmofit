"""Compare CosmoFit and direct-Cobaya Union3 chains."""
...
def newest_cosmofit_root() -> Path:
    parent = Path("outputs/validation_union3_lcdm")
    ...
    if not run_directories:
        raise FileNotFoundError(
            "No CosmoFit Union3 validation runs found."
        )
...
def summarize(root: Path, label: str) -> dict[str, float]:
    ...
    parameter = samples.getMargeStats().parWithName("Om")
...
def normalized_mean_difference(
    ...
    return abs(first_mean - second_mean) / combined_sigma
