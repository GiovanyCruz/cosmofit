"""Independent Cobaya validation for flat LCDM with Pantheon+."""
...
OUTPUT_ROOT = (
    ...
    / "outputs"
    / "direct_cobaya_pantheonplus_lcdm"
    / "chain"
)
    ...
    def must_provide(
        ...
        **requirements: Any,
        ...
        if "angular_diameter_distance" in requirements:
            ...
            if redshifts.ndim != 1:
                raise ValueError(
                    "Pantheon+ redshifts must be one-dimensional."
                )
...
def expansion_rate_ratio(z: float, om: float) -> float:
...
def main() -> None:
    """Execute the independent Pantheon+ MCMC run."""
    ...
    print("Independent Pantheon+ Cobaya run completed.")
