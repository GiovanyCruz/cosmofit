"""Independent Cobaya validation for flat LCDM with Pantheon+SH0ES."""
...
class IndependentFlatLCDMTheory(Theory):
    ...
    def initialize(self) -> None:
        """Initialize storage for requested theory redshifts."""

        self._requested_redshifts: dict[str, np.ndarray] = {}

    def get_can_provide(self) -> list[str]:
        """Declare the theory quantity provided to Cobaya."""
    ...
    def must_provide(
        ...
        **requirements: Any,
        ...
        """Store the redshifts requested by Pantheon+SH0ES."""
        ...
        if "angular_diameter_distance" in requirements:
            ...
            if redshifts.ndim != 1:
                raise ValueError(
                    "Pantheon+SH0ES redshifts must be one-dimensional."
                )
    ...
    def calculate(
        ...
        **params_values_dict: float,
        ...
        """Calculate angular-diameter distances for one parameter point."""
        ...
        state["angular_diameter_distance"] = distances
        return True
    ...
    def get_angular_diameter_distance(
        ...
        """Return distances previously calculated for requested redshifts."""
        ...
        available_distances = np.asarray(
        ...
        result = available_distances[indices]
...
def angular_diameter_distance(
    *,
    ...
    """Return flat-LCDM angular-diameter distance in Mpc."""
...
def main() -> None:
    """Execute the independent Pantheon+SH0ES MCMC run."""
    ...
    print(
        "Independent Pantheon+SH0ES Cobaya run completed."
    )
    print(f"Number of rows: {len(sample)}")
    print(f"Output root: {OUTPUT_ROOT}")
    print(
        f"Updated keys: {sorted(updated_info.keys())}"
    )
